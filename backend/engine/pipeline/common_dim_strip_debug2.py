...
def run_common_dim_strip_then_rectpack(
    ...
    if cand_dim <= board_h + bt:
        strip_height = cand_dim
        strip_max_width = board_w + bt
        print(f"DEBUG: board_w={board_w}, bt={bt}, max={strip_max_width}, plates_len={big_plates[0].length}")
    else:
        strip_height = cand_dim
        strip_max_width = board_h + bt
        ...
        for nx in range(1, 4):
            for ny in range(2, 6):
                for ow, oh in [(w1, h1), (h1, w1)]:
                    bw = nx * ow + (nx - 1) * bt
                    bh = ny * oh + (ny - 1) * bt
                    if bh <= cand_dim and bw <= strip_max_width:
                        area = nx * ny * w1 * h1
                        util = area / (bw * bh)
                        if util >= 0.85 and bh >= cand_dim * 0.95:
                            macros.append({
                                'w1d': bw + bt,
                                'stack_h': bh,
                                'util': util,
                ...
                for ny1 in range(1, 4):
                    ...
                    for ow2, oh2 in [(w2, h2), (h2, w2)]:
                        for nx2 in range(1, 4):
                            ...
                            if max(bw1, bw2) > strip_max_width:
                            ...
                            for ny2 in range(1, 4):
                                ...
                                if util >= 0.85 and stack_h >= cand_dim * 0.95:
                                    macros.append({
                                        'w1d': stack_w + bt,
                                        'stack_h': stack_h,
                                        'util': util,
